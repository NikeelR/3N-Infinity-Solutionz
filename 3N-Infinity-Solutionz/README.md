...
Deploy to Render/Railway/a VPS using the `backend/` folder as the service root. A `render.yaml` is included — on Render, connect the repo and it will auto-configure `rootDir: backend`. The Express server serves the `frontend/` folder as static files, so one deployment hosts the whole site and the contact form works same-origin with zero config.

On Render manually: set **Root Directory** to `backend`, **Build Command** to `npm install`, **Start Command** to `npm start`.
