...
| Platform | Settings |
|---|---|
| **Vercel** | Root Directory = **`.`** (repo root, not `backend`). Push `vercel.json` — it routes all traffic through the Express app, which serves `frontend/` and handles `/api/*`. Redeploy after pushing. |
| **Render** | Root Directory = `backend`, Start Command = `npm start`. A `render.yaml` is included for auto-config. |
| **Railway / VPS** | Deploy `backend/`, run `npm start`. Express serves the whole site from `frontend/`. |

The Express server serves the `frontend/` folder as static files, so one deployment hosts the whole site and the contact form works same-origin with zero config.
